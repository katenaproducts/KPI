...
/****** Object:  StoredProcedure [dbo].[EXTGEN_Rpt_PackingSlipSp]    Script Date: 08/30/2017 15:48:44 ******/
SET ANSI_NULLS ON
GO
 ...
 * $NoKeywords: $
 */
CREATE PROCEDURE [dbo].[EXTGEN_Rpt_PackingSlipSp] (
...
-- _KPI
 -- update total qty _shipped, total qty_packed, Open Qty
--
update pckitem
set Uf_TotalQtyPacked   = coitem.qty_packed,
    Uf_TotalQtyShipped  = coitem.qty_shipped,
    Uf_QtyOpen			= coitem.qty_ordered_conv - coitem.qty_packed + pckitem.qty_packed

    from pckitem join coitem (nolock) on 
		coitem.co_num = pckitem.co_num and
		coitem.co_line = pckitem.co_line 
		
		
		where pckitem.pack_num >= @MinPackNum
   AND   pckitem.pack_num <= @MaxPackNum
		
update _KPI_BarCodes
set Pack_Num = pck.pack_num from 
_KPI_BarCodes bc
 join pckitem pck (nolock) on
 pck.co_num = bc.RefNum and
 pck.co_line = bc.RefLine and
 pck.pack_num in
 (select MAX(pack_num) from pckitem pck1 (nolock)
  where 
    pck1.co_num = pck.co_num and 
    pck1.co_line = pck.co_line and
    pck1.co_release = pck.co_release
   )
   and  pck.pack_num >= @MinPackNum
   AND   pck.pack_num <= @MaxPackNum
...
insert @KPI_PackingInfo select distinct 
	rs.pack_num, rs.co_num, rs.co_line, rs.co_release, ''
	from @reportset rs
...
select 
 ...
 + case 
  ...
  0
from 
         _KPI_BarCodes bc where bc.pack_num >= @MinPackNum
   AND   bc.pack_num <= @MaxPackNum


declare @KPISerialLotString nvarchar(100)
declare @KPIPackNum integer
...
begin
	...
	select 
	  ...
	  if ISNULL(@KPIPackNum,0) = 0 break
	  update @KPIBC
...
SELECT
  isnull(pckinfo.SerialLotString, '') as  SerialLot -- 
, pck.Uf_TotalQtyPacked
, pck.Uf_TotalQtyShipped
, pck.Uf_QtyOpen
, rs.pack_num
, pack_date
, whse
...
FROM @ReportSet rs 
	...
	join pckitem pck on 
	pck.pack_num = rs.pack_num and 
	pck.co_num = rs.co_num and
	pck.co_line = rs.co_line and
	pck.co_release = 0 

    left outer join @KPI_PackingInfo pckinfo on 
			pckinfo.PackNum = rs.pack_num and
			pckinfo.CoNum = rs.co_num and
			pckinfo.CoLine = rs.co_line	
 

ORDER BY rs.pack_num, rs.co_num ASC, rs.co_line ASC, rs.co_release,Kit_Flag ASC
