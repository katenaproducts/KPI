...
/****** Object:  StoredProcedure [dbo].[EXTGEN_CLM_OrderShippingSp]    Script Date: 08/30/2017 15:48:08 ******/
SET ANSI_NULLS ON
GO
 ...
 * $NoKeywords: $
 ...
 -- _KPI May 2017 Only Locations that can be shipped from are listed on the output
 */
CREATE PROCEDURE [dbo].[EXTGEN_CLM_OrderShippingSp] (
...
begin
   ...
   BEGIN
      ...
      -- _KPI only shippable locations
      and itemloc.loc in (select loc from location (nolock)where location.Uf_AllowedToShipFrom = 1)
      -- _KPI only shippable locations
   ...
   BEGIN
      ...
      BEGIN
         ...
         and lot_loc.lot <> 'UNIX' -- _KPI
          -- _KPI only shippable locations
      and lot_loc.loc in (select loc from location (nolock)where location.Uf_AllowedToShipFrom = 1)
      -- _KPI only shippable locations
      ...
      IF @ItemLotTracked <> 0
         ...
         BEGIN
            ...
            BEGIN
               ...
               AND lot_loc.loc = @ItemlocLoc and
               lot_loc.lot<>'UNIX' -- _KPI
               AND lot_loc.qty_on_hand - lot_loc.qty_contained > 0.0
               order by lot_loc.whse asc, lot_loc.item asc, lot_loc.lot asc, lot_loc.loc asc
               if @@rowcount <> 1
                  set @LotLocRowpointer = null
            ...
            begin
               ...
               from lot_loc
                  where 
                  lot_loc.lot <> 'UNIX' and
                  ( lot_loc.whse = @ItemlocWhse
                  AND lot_loc.item = @ItemlocItem
                  AND lot_loc.loc = @ItemlocLoc
                  AND lot_loc.qty_on_hand - lot_loc.qty_contained > 0.0
                 ) and (
