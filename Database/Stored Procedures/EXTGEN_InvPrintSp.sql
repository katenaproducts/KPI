...
/****** Object:  StoredProcedure [dbo].[EXTGEN_InvPrintSp]    Script Date: 08/30/2017 15:48:31 ******/
SET ANSI_NULLS ON
GO
 ...
 * $NoKeywords: $
 */
CREATE PROCEDURE [dbo].[EXTGEN_InvPrintSp] (
...
-- _KPI update barcode table with the Invoice Number

update _KPI_BarCodes
set InvNum = @TPrintInvNum
from pckitem pck (nolock) join _KPI_BarCodes bc
on bc.Pack_Num = pck.pack_num 

where pck.inv_num = @TPrintInvNum
...
--_kpi
  select @InvHdrCustPo = co.cust_po from co(nolock) where co.co_num = @InvHdrCoNum
--_kpi
...
--   SET @Addr1 = dbo.FormatAddressWithContactSp(@InvHdrCustNum, 0, @BillToContact)
   SET @Addr1 = dbo.FormatAddress(@InvHdrCustNum, 0)
...
--   SET @Addr2 = dbo.FormatAddressWithContactSp(@InvHdrCustNum, @InvHdrCustSeq, @ShipToContact)
   SET @Addr2 = dbo.FormatAddress(@InvHdrCustNum, @InvHdrCustSeq)
...
--      SET @Addr1 = dbo.FormatAddressWithContactSp(@TCorpCust, 0, @BillToContact)
        SET @Addr1 = dbo.FormatAddress(@TcorpCust, 0)
   END
...
--
   ...
   --_KPI -- Slsman number rather than name
...
-- _KPI -- update totals on all records

update #tt_invoice_credit_memo
set 
	inv_sale_amt = @TcAmtTotalPrice,
    inv_disc_amt = @TDiscMask,
    inv_net_amt = @TcAmtNetMask,
    inv_freight = @TcAmtFreight,
    inv_sales_tax = @TcAmtSalesTax,
    inv_sales_tax2 =@TcAmtSalesTax2,
    inv_prepaid_amt = @TcAmtPrepaidAmt,
    inv_total = @TcAmtPrice,
    amt_total = @AmtTotal
    
from  #tt_invoice_credit_memo tt where tt.inv_num = @InvHdrInvNum
