...
/****** Object:  StoredProcedure [dbo].[Rpt_OrderInvoicingCreditMemoSp]    Script Date: 09/06/2017 11:23:34 ******/
SET ANSI_NULLS ON
GO
 ...
 * $NoKeywords: $
 */
CREATE PROCEDURE [dbo].[Rpt_OrderInvoicingCreditMemoSp](
...
SELECT tt.*, coi.promotion_code,@CoSurcharge AS surcharge,it.item_content, dbo.DisplayAddressForReportFooter() AS ParmsSingleLineAddress,it.drawing_nbr,
coi.delterm, coi.ec_code, coi.origin, coi.comm_code, del_term.description, itemcust.end_user, ari.due_date, bank_hdr.name AS t_bank_name , bank_hdr.bank_transit_num AS t_bank_transit_num
,bank_hdr.bank_acct_no AS t_bank_acct_no
FROM #tt_invoice_credit_memo tt
LEFT JOIN item AS it ON tt.item1 = it.item
Left join coitem coi on tt.co_num = coi.co_num and substring(tt.str_co_line,1,4) = coi.co_line and substring(tt.str_co_line,7,4) = coi.co_release
LEFT JOIN itemcust ON itemcust.cust_item = coi.cust_item AND itemcust.item = coi.item AND itemcust.cust_num = coi.co_cust_num
